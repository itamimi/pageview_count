...
dag = DAG('bi_pageview_postcode_count_hourly',
          schedule_interval='@hourly',  #  run it hourly after pageviews extract
          catchup=False,
          max_active_runs=1,
...
def bi_fnc_run_pageviews_user_history(ds, **kwargs):
    ...
    bi_fnc_run_sql_script('insert_Pageviews_user_history.sql',db1_cursor)
...
wait_for_pageviews_extract = ExternalTaskSensor(
        task_id='wait_for_pageviews_extract',
        external_dag_id='pageviews_extract_hourly',
        external_task_id='run_pageviews_extract_hourly',  # assume the task is named that way in  insert_pageviews_extract dag.
...
wait_for_pageviews_extract >> run_pageviews_user_history_hourly >> run_pageviews_postcode_count_hourly
